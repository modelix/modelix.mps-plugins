...
class TransientModuleBinding(moduleNodeId: Long) : ModuleBinding(moduleNodeId, SyncDirection.TO_MPS) {
    ...
    override fun doActivate() {
        ...
        if (moduleIdStr != null) {
            ...
            if (CloudTransientModules.Companion.getInstance().isModuleIdUsed(temptativeModuleId)) {
        ...
        module = CloudTransientModules.Companion.getInstance().createModule(moduleName, moduleId)
        super.doActivate()
    }
            ...
            public override fun run() {
                synchronized(this@TransientModuleBinding, {
                    MPSModuleRepository.getInstance().getModelAccess().runWriteAction(object : Runnable {
                        public override fun run() {
                            CloudTransientModules.Companion.getInstance().disposeModule(module)
                        }
                    })
                })
            }
    ...
    protected override val modelsSynchronizer: Synchronizer<SModel?>
        protected get() {
            return TransientModelsSynchronizer(getModuleNodeId(), module)
        }
