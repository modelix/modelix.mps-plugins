...
class ForeignNodeReference(val modelName: String, val foreignId: String) : INodeReference {
    ...
    private inner class SimpleSNodeReference(
        ...
        override fun resolve(repository: SRepository): SNode? {
            val model: SModel = SPointerOperations.resolveModel(modelReference, repository)
    ...
    fun getReference(repository: SRepository?): SNodeReference {
        ...
        for (module: SModule? in Sequence.fromIterable<SModule>(repository.modules)) {
            for (itModel: SModel? in ListSequence.fromList<SModel?>(SModuleUtils.getModelsWithoutDescriptor(module))) {
                if (Objects.equals(itModel!!.modelName, modelName)) {
                    sModelReference = itModel.reference
                }
            }
        }
    ...
    class ForeignNodeReferenceSerializer : INodeReferenceSerializer {
        override fun serialize(nr: INodeReference): String? {
        ...
        override fun deserialize(refValueAsString: String): INodeReference? {
