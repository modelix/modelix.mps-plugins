...
class CloudNodeTreeNode(val branch: IBranch, val node: INode) : TextTreeNode("") {
    ...
    fun updateText() {
        ...
        SharedExecutors.FIXED.execute(_Adapters._return_P0_E0_to_Runnable_adapter(object : _return_P0_E0<Unit> {
                ...
                return PArea(branch).executeRead({
                    ...
                    if (nodeId == ITree.ROOT_ID) {
                        ...
                        if (concept != null) {
                            ...
                            val snode: SNode? = wrap(node)
                            val mpsRepo: SRepository? = CommandHelper.getSRepository()
                            if (mpsRepo == null) {
                                throw IllegalStateException("repository should not be null")
                            }
                            mpsRepo.getModelAccess().runReadAction(_Adapters._return_P0_E0_to_Runnable_adapter(object :
                                _return_P0_E0<Unit> {
                                public override fun invoke() {
                                    return@executeRead withAdditionalContext<Unit>(MPSArea(mpsRepo), {
                                        try {
                                            newText.value =
            ...
            public override fun invoke() {
                return PArea(branch).executeRead<Unit>({
                    ...
                    val newChildren: List<CloudNodeTreeNode> = Sequence.fromIterable<INode>(allChildren)
                        .select<CloudNodeTreeNode>(object : ISelector<INode, CloudNodeTreeNode>() {
                            public override fun select(it: INode): CloudNodeTreeNode {
                                return@executeRead (if (MapSequence.fromMap<INode, CloudNodeTreeNode>(existing)
                                        .containsKey(it)
                                ) MapSequence.fromMap<INode, CloudNodeTreeNode>(existing)
                                    .get(it) else CloudNodeTreeNode(
                                    branch, it
                                ))
                            }
                        }).toListSequence()
                    ThreadUtils.runInUIThreadNoWait(object : Runnable {
                        public override fun run() {
                            TreeModelUtil.setChildren(
                                this@CloudNodeTreeNode, ListSequence.fromList(newChildren).ofType(
                                    TreeNode::class.java
                                )
                            )
