...
/*Generated by MPS */
class LoadHistoryForBranch_Action() : BaseAction("Show History", "", ICON) {
    init {
    ...
    public override fun doExecute(event: AnActionEvent, _params: Map<String, Any>) {
        ...
        val infoBranch: IBranch? = treeTNode.getModelServer().getInfoBranch()
        val branchName: String = PArea((infoBranch)!!).executeRead({
            SPropertyOperations.getString(
                branchTNode.getBranchInfo(),
                PROPS.`name$MnvL`
            )
        })
        val repositoryId: RepositoryId? = treeTNode.getRepositoryId()
        val branchKey: String = repositoryId!!.getBranchKey(branchName)
        val modelServer: ModelServerConnection? = treeTNode.getModelServer()
