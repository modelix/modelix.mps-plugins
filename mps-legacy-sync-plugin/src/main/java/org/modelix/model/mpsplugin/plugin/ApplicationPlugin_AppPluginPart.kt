...
class ApplicationPlugin_AppPluginPart() : ApplicationPluginPart() {
        ...
        try {
            ...
            if (executionMode == EModelixExecutionMode.CLUSTER || executionMode == EModelixExecutionMode.PROJECTOR) {
                ...
                if ((modelUri != null && modelUri.length > 0)) {
                    ModelServerConnections.Companion.getInstance().addModelServer(modelUri)
                }
                ...
                autoBindings =
                    AutoBindings(ModelServerConnections.Companion.getInstance(), fixedRepositoryIds, bindProjects)
            }
            when (executionMode) {
                ...
                EModelixExecutionMode.DEFAULT -> {
                    ...
                    if (cloudReposSpecified != null && (cloudReposSpecified != null && cloudReposSpecified.length > 0)) {
                        ...
                        for (repo: String? in repos) {
                            ModelServerConnections.Companion.getInstance().addModelServer(repo)
                        }
                    }
                }
    ...
    public override fun dispose() {
        ...
        try {
            ModelServerConnections.Companion.getInstance().dispose()
        } catch (ex: Exception) {
            if (LOG.isEnabledFor(Level.ERROR)) {
                LOG.error("", ex)
            }
        ...
        try {
            CloudTransientModules.Companion.getInstance().dispose()
        } catch (ex: Exception) {
            if (LOG.isEnabledFor(Level.ERROR)) {
                LOG.error("", ex)
            }
    ...
    companion object {
        private val LOG: Logger = LogManager.getLogger(ApplicationPlugin_AppPluginPart::class.java)
    }
}
