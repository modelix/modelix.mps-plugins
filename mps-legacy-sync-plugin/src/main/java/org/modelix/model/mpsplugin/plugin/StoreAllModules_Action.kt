...
class StoreAllModules_Action() : BaseAction("Store Entire MPS Module Repository", "", ICON) {
    ...
    public override fun doExecute(event: AnActionEvent, _params: Map<String, Any>) {
        ...
        val branch: IBranch = treeInRepository.getActiveBranch().branch
        val task: Task.Backgroundable = object : Task.Backgroundable(
            ...
            public override fun run(indicator: ProgressIndicator) {
                ...
                mpsRepo.getModelAccess().runReadAction(object : Runnable {
                    public override fun run() {
                        mpsModules.value = ListSequence.fromListWithValues(ArrayList(), mpsRepo.getModules())
                    }
                })
                val existingModules: Set<String?>? = branch.computeRead<ISetSequence<String?>?>({
                    val t: ITransaction = branch.transaction
                    val existingModules: Iterable<Long> = t.getChildren(cloudProjectId, LINKS.`modules$Bi3g`.getName())
                    SetSequence.fromSetWithValues(
                        ...
                        Sequence.fromIterable(existingModules).select(object : ISelector<Long?, String>() {
                            public override fun select(it: Long?): String {
                                return@computeRead t.getProperty((it)!!, PROPS.`name$MnvL`.getName())
                            }
                        })
                    )
                })
