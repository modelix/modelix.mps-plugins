...
import org.modelix.model.mpsplugin.RootBinding
...
abstract class Binding(protected var initialSyncDirection: SyncDirection?) {
    ...
    fun createTask(direction: SyncDirection, initial: Boolean): SyncTask {
        when (direction) {
            SyncDirection.TO_CLOUD -> return SyncTask(this, direction, initial, SetSequence.fromSetAndArray(
                HashSet(), ELockType.MPS_READ, ELockType.CLOUD_WRITE
            ), object : Runnable {
    ...
    val allBindings: Iterable<Binding>
        get() {
            return Sequence.fromIterable(Sequence.singleton(this))
                .concat(SetSequence.fromSet(ownedBindings).translate(object : ITranslator2<Binding, Binding>() {
