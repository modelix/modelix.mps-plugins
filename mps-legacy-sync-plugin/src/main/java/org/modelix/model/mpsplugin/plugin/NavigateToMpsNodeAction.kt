...
/*Generated by MPS */
class NavigateToMpsNodeAction : BaseAction("Navigate to Corresponding MPS Node", "", ICON) {
    init {
    ...
    public override fun doExecute(event: AnActionEvent, _params: Map<String, Any>) {
        ...
        treeInRepository.runRead(object : Runnable {
            override fun run() {
                ...
                val moduleId: String? = cloudModule.getPropertyValue(PROPS.`id$7MjP`.name)
                if (moduleId == null) {
                    Messages.showErrorDialog(
                        event.getData(CommonDataKeys.PROJECT),
                        "No module id",
                ...
                val modelId: String? = cloudModel.getPropertyValue(PROPS.`id$lDUo`.name)
                if (modelId == null) {
                    Messages.showErrorDialog(
                        event.getData(CommonDataKeys.PROJECT),
                        "No model id",
                ...
                val repo: SRepository = ProjectHelper.toMPSProject(event.getData(CommonDataKeys.PROJECT))!!
                    .repository
                repo.modelAccess.runReadAction(object : Runnable {
                    override fun run() {
                        for (module: SModule in Sequence.fromIterable<SModule>(repo.modules)) {
                            if (Objects.equals(module.moduleId.toString(), moduleId)) {
                                for (model: SModel in Sequence.fromIterable<SModel>(module.models)) {
                                    if (Objects.equals(model.modelId.toString(), modelId)) {
    ...
    fun findNodeInModel(model: SModel, nodeId: String?, event: AnActionEvent?): SNode? {
        for (root: SNode in Sequence.fromIterable(model.rootNodes)) {
    ...
    fun findNodeInNode(node: SNode, nodeId: String?, event: AnActionEvent?): SNode? {
        if (Objects.equals(node.nodeId.toString(), nodeId)) {
            return node
        }
        for (child: SNode in Sequence.fromIterable(node.children)) {
