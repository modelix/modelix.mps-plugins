...
class ModelServerConnection @JvmOverloads constructor(baseUrl: String, providedHttpClient: HttpClient? = null) {
    ...
    init {
        ...
        val workspaceTokenProvider: Function0<String?> = { InstanceJwtToken.token }
        val tokenProvider: Function0<String>? =
            (if (ModelixConfigurationSystemProperties.getExecutionMode() == EModelixExecutionMode.PROJECTOR) workspaceTokenProvider else null)
        ...
        client.addStatusListener({ oldStatus: ConnectionStatus?, newStatus: ConnectionStatus ->
    ...
    val isConnected: Boolean
        get() {
            val status: ConnectionStatus? = check_v06aqd_a0a0fb(client)
    ...
    protected fun checkConnected() {
        val connectionStatus: ConnectionStatus? = check_v06aqd_a0a0jb(client)
            ...
            ListSequence.fromList(SLinkOperations.getChildren(modelServerInfo, LINKS.`repositories$b56J`))
                .findFirst(object : IWhereFilter<SNode?>() {
                    public override fun accept(it: SNode?): Boolean {
                        return@executeRead Objects.equals(
                ...
                ListSequence.fromList(SLinkOperations.getChildren(modelServerInfo, LINKS.`repositories$b56J`))
                    .select(object : ISelector<SNode?, String>() {
                        public override fun select(it: SNode?): String {
                            return@executeRead SPropertyOperations.getString(it, PROPS.`id$baYB`)
                        }
                    }).toListSequence()
            })
    ...
    fun addRepository(id: String?): SNode {
        return PArea((infoBranch)!!).executeWrite({
            ...
            val repositoryInfo: SNode = SNodeOperations.cast(
                addNewChild(modelServerInfo, LINKS.`repositories$b56J`),
                CONCEPTS.`RepositoryInfo$5Y`
            )
            SPropertyOperations.assign(repositoryInfo, PROPS.`id$baYB`, id)
            val branchInfo: SNode =
                SNodeOperations.cast(addNewChild(repositoryInfo, LINKS.`branches$b5_g`), CONCEPTS.`BranchInfo$6t`)
        ...
        PArea((infoBranch)!!).executeWrite<SNode>({
            ...
            SNodeOperations.deleteNode(
                ListSequence.fromList(
                    ...
                    public override fun accept(it: SNode?): Boolean {
                        return@executeWrite Objects.equals(SPropertyOperations.getString(it, PROPS.`id$baYB`), id)
                    }
                })
            )
        })
        ...
        return Sequence.fromIterable(projectBindings).any(object : IWhereFilter<ProjectBinding>() {
            public override fun accept(it: ProjectBinding): Boolean {
                return Objects.equals(
                    it.getCloudRepository().getRepositoryId(),
                    repositoryId
                ) && Objects.equals(it.getProjectNodeId(), projectNodeId)
            }
        })
    }
        ...
        return Sequence.fromIterable(
            ...
            public override fun accept(it: ModuleBinding): Boolean {
                return Objects.equals(
                    it.getCloudRepository().getRepositoryId(),
                    repositoryId
                ) && Objects.equals(it.getModuleNodeId(), moduleNodeId)
            }
        })
    }
                ...
                val allChildren: Iterable<SNode> =
                    Sequence.fromIterable<Long>(allChildren_).select<SNode>(object : ISelector<Long?, SNode>() {
                        public override fun select(it: Long?): SNode {
                            return@executeRead wrap(PNodeAdapter((it)!!, infoTree!!.branch)) as SNode?
                        }
                    })
                Sequence.fromIterable(SNodeOperations.ofConcept(allChildren, CONCEPTS.`ModelServerInfo$5v`)).first()
            })
            if (result == null) {
                SharedExecutors.FIXED.submit(_Adapters._return_P0_E0_to_Runnable_adapter(object : _return_P0_E0<SNode> {
                    public override fun invoke(): SNode {
                        return PArea(infoTree!!.branch).executeWrite<SNode>({
                            ...
                            val repoInfo: SNode = SNodeOperations.cast(
                                wrap(PNodeAdapter(id, infoTree!!.branch)),
    ...
    fun getActiveBranch(repositoryId: RepositoryId?): ActiveBranch {
        ...
        synchronized(activeBranches, {
            ...
            if (ab == null) {
                ab = object : ActiveBranch(client, (repositoryId)!!, null, { author }) {
                    protected override fun createReplicatedRepository(
                        client: IModelClient,
                        repositoryId: RepositoryId,
                        branchName: String,
                        user: Function0<String>
                    ): ReplicatedRepository {
                        return MpsReplicatedRepository(client, repositoryId, branchName, object : Supplier<String> {
                            public override fun get(): String {
                                return user.invoke()
                            }
                        })
    ...
    companion object {
        ...
        private fun check_v06aqd_a0a0fb(checkedDotOperand: RestWebModelClient?): ConnectionStatus? {
        ...
        private fun check_v06aqd_a0a0jb(checkedDotOperand: RestWebModelClient?): ConnectionStatus? {
            if (null != checkedDotOperand) {
                return checkedDotOperand.connectionStatus
            }
            return null
