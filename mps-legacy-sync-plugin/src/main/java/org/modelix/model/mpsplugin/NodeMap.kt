...
class NodeMap(private val branchProvider: BranchProvider) : AbstractArea(), IAreaReference {
    open fun interface BranchProvider {
        val branch: IBranch?
    }
    ...
    public override fun resolveOriginalNode(reference: INodeReference?): INode? {
        if (reference is SNodeReferenceAdapter) {
            ...
            val sNode: Optional<SNode?> = node2id.keySet().stream().filter(object : Predicate<SNode> {
                public override fun test(sNode: SNode): Boolean {
                    return Objects.equals(sNode.getNodeId(), targetNodeId)
                }
            }).findFirst()
            if (sNode.isPresent()) {
                return SNodeToNodeAdapter.Companion.wrap(sNode.get())
            } else {
                return null
            }
        }
