...
class ModelServerConnections() {
    private val modelServers: List<ModelServerConnection?> = ListSequence.fromList(ArrayList())
    ...
    fun addModelServer(url: String?, providedHttpClient: HttpClient? = null): ModelServerConnection? {
        ...
        val result: ModelServerConnection? = doAddModelServer(url, providedHttpClient)
    ...
    protected fun doAddModelServer(url: String, providedHttpClient: HttpClient?): ModelServerConnection? {
        val newRepo: ModelServerConnection? =
    ...
    fun ensureModelServerIsPresent(url: String?): ModelServerConnection? {
        val modelServerConnection: ModelServerConnection? = getModelServer(url)
        if (modelServerConnection != null) {
            return modelServerConnection
        } else {
            return instance.addModelServer(url)
        }
    }
