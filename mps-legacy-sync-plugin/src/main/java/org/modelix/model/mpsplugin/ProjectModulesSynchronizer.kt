...
class ProjectModulesSynchronizer(cloudParentId: Long, val project: MPSProject) :
    Synchronizer<SModule>(cloudParentId, LINKS.`modules$Bi3g`.getName()) {

    override val mPSChildren: Iterable<SModule>
        get() {
            val projectModules: List<SModule> = project.getProjectModules()
    ...
    public override fun removeMPSChild(mpsChild: SModule) {
    ...
    public override fun associate(
        ...
        for (cloudModuleId: Long in cloudChildren) {
            ...
            val name: String? = tree.getProperty(cloudModuleId, PROPS.`name$MnvL`.getName())
    ...
    protected fun getModuleId(tree: ITree, cloudModuleId: Long): SModuleId? {
        val serializedId: String? = tree.getProperty(cloudModuleId, PROPS.`id$7MjP`.getName())
    ...
    public override fun createCloudChild(t: IWriteTransaction, mpsChild: SModule): Long {
        return createModuleOnCloud(t, mpsChild, cloudParentId, LINKS.`modules$Bi3g`.getName())
    }
