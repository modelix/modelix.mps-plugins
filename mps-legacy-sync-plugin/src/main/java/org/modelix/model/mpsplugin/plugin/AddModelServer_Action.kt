...
/*Generated by MPS */
class AddModelServer_Action() : BaseAction("Add Model Server", "", ICON) {
    init {
    ...
    public override fun doExecute(event: AnActionEvent, _params: Map<String, Any>) {
        ...
        if (url != null && (url != null && url.length > 0)) {
            ...
            if (ModelServerConnections.Companion.getInstance().existModelServer(url)) {
...
//      } else {
            val modelServer: ModelServerConnection? =
                ModelServerConnections.Companion.getInstance().ensureModelServerIsPresent(finalUrl)
    ...
    companion object {
        private val LOG: Logger = LogManager.getLogger(AddModelServer_Action::class.java)
