...
class NodeToSModelAdapter protected constructor(node: INode, repository: SRepository?) : SModel {
    ...
    public override fun getRootNodes(): Iterable<SNode> {
        val rootNodes: Iterable<INode> = node!!.getChildren(LINKS.`rootNodes$jxXY`.getName())
        return Sequence.fromIterable(rootNodes).select(object : ISelector<INode?, SNode?>() {
            public override fun select(it: INode?): SNode? {
                val adapter: SNode? = wrap(it)
