...
class ANode(private val node: SNode?) : jetbrains.mps.smodel.SNode(DummyConcept(), DummySNodeId()), SNode {
    ...
    public override fun getChildren(link: SContainmentLink): List<jetbrains.mps.smodel.SNode> {
        val children: Iterable<SNode> = node!!.getChildren(link)
        return Sequence.fromIterable(children).select(object : ISelector<SNode?, jetbrains.mps.smodel.SNode?>() {
            public override fun select(it: SNode?): jetbrains.mps.smodel.SNode? {
                return wrap(it)
            }
        }).toListSequence()
    }
    ...
    public override fun getChildren(role: String): List<jetbrains.mps.smodel.SNode> {
        val children: Iterable<SNode> = node!!.getChildren(role)
        return Sequence.fromIterable(children).select(object : ISelector<SNode?, jetbrains.mps.smodel.SNode?>() {
            public override fun select(it: SNode?): jetbrains.mps.smodel.SNode? {
                return it as jetbrains.mps.smodel.SNode?
            }
        }).toListSequence()
    }
