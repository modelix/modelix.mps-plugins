...
class ProjectModuleAsNode(project: MPSProject?, module: SModule?) :
    TreeElementAsNode<Tuples._2<MPSProject, SModule>?>(MultiTuple.from<MPSProject?, SModule?>(project, module)) {
    private val moduleAccessor: IReferenceAccessor<Tuples._2<MPSProject, SModule>?> =
        object : IReferenceAccessor<Tuples._2<MPSProject?, SModule>> {
            public override fun get(element: Tuples._2<MPSProject?, SModule>): INode? {
                return SModuleAsNode(element._1())
            }

            public override fun set(element: Tuples._2<MPSProject?, SModule>, target: INode?): INode? {
                throw UnsupportedOperationException("readonly")
            }
        }
    private val virtualFolderAccessor: IPropertyAccessor<Tuples._2<MPSProject, SModule>?> =
        object : IPropertyAccessor<Tuples._2<MPSProject, SModule?>> {
            public override fun get(element: Tuples._2<MPSProject, SModule?>): String? {
                return check_9jclrw_a0a0a0a1(element._0().getPath((element._1())!!))
            }

            public override fun set(element: Tuples._2<MPSProject, SModule?>, value: String?): String? {
                element._0().setVirtualFolder((element._1())!!, value)
                return value
            }
        }

    override fun getReferenceAccessor(role: String): IReferenceAccessor<Tuples._2<MPSProject, SModule>?>? {
    ...
    override fun getPropertyAccessor(role: String): IPropertyAccessor<Tuples._2<MPSProject, SModule>?>? {
    ...
    public override fun getConcept(): IConcept? {
        return SConceptAdapter.Companion.wrap(CONCEPTS.`ProjectModule$uf`)
    }

    public override fun getParent(): INode {
        return ProjectAsNode(getElement()._0())
    }

    public override fun getReference(): INodeReference {
        return NodeReference(parent!!.reference as ProjectAsNode.NodeReference?, getElement()._1().getModuleReference())
    }
    ...
    class NodeReference(
        ...
        public override fun resolveNode(area: IArea?): INode? {
            val project: MPSProject? = projectRef!!.resolveNode(area).getElement()
            val module: SModule? = moduleRef!!.resolve(project!!.getRepository())
