...
/*Generated by MPS */
class NodeToSNodeAdapter protected constructor(node1: INode, repository: SRepository?) : SNode {
    ...
    init {
        if (node1.concept == null) {
            throw RuntimeException("Node has no concept: " + node1)
        }
        node = node1
    ...
    public override fun getConcept(): SConcept {
        ...
        if (concept is SConceptAdapter) {
            return concept.getAdapted() as SConcept
        }
    ...
    public override fun getReferences(): Iterable<SReference> {
        val links: Iterable<SReferenceLink> = SConceptAdapter.Companion.unwrap(
            node!!.concept
        )!!.getReferenceLinks()
        return Sequence.fromIterable(links).where(object : IWhereFilter<SReferenceLink>() {
            public override fun accept(it: SReferenceLink): Boolean {
                return getReferenceTarget(it) != null
            }
        }).select(object : ISelector<SReferenceLink?, Reference>() {
            public override fun select(it: SReferenceLink?): Reference {
                return Reference(it)
            }
        })
    }
    ...
    public override fun putUserObject(key: Any, value: Any?) {
        if (userObjects != null) {
            ...
            while (i < userObjects!!.size) {
                if (Objects.equals(userObjects!!.get(i), key)) {
                    userObjects!!.get(i + 1) = (value)!!
                    return
                }
                i += 2
            }
            val newArray: Array<Any?> = arrayOfNulls(userObjects!!.size + 2)
            System.arraycopy(userObjects, 0, newArray, 0, userObjects!!.size)
            newArray.get(newArray.size - 2) = key
            newArray.get(newArray.size - 1) = value
